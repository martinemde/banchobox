			...
			<Accordion.Item value="recipe">
				{#snippet control()}
					<div class="flex w-full items-center justify-between">
						<span class="truncate text-xs opacity-80">{ingredientSummary}</span>
					</div>
				{/snippet}
						...
						<table class="w-full table-auto text-sm">
								...
								{#each ingredientRows as row}
									<tr class="border-b border-surface-200-800">
										...
										<td class="p-2 text-right tabular-nums"
											>{row.buy == null ? '—' : formatNumber(row.buy)}</td
										>
										<td class="p-2 text-right tabular-nums"
											>{row.sell == null ? '—' : formatNumber(row.sell)}</td
										>
							...
							<tfoot>
								<tr class="bg-surface-200-800 font-semibold">
									...
									<td class="p-2 text-center tabular-nums"
										>{dish.upgradeCost ? formatNumber(dish.upgradeCost) : '—'}</td
									>
									<td></td>
