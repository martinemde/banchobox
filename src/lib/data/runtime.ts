...
const ingredientById = Object.fromEntries(ingredients.map((i) => [i.id, i])) as Record<
	Id,
	EnrichedIngredient
>;
const partyById = Object.fromEntries(parties.map((p) => [p.id, p])) as Record<Id, EnrichedParty>;
const partyDishById = Object.fromEntries(partyDishes.map((pd) => [pd.id, pd])) as Record<
	Id,
	PartyDish
>;
...
export const Data = {
	...
	getDishesByPartyId: (partyId: Id): EnrichedDish[] => {
		const partyDishIds = partyDishes.filter((pd) => pd.partyId === partyId).map((pd) => pd.dishId);
	...
	searchDishesByName: (query: string): EnrichedDish[] =>
		dishes.filter((dish) => dish.name.toLowerCase().includes(query.toLowerCase())),

	searchIngredientsByName: (query: string): EnrichedIngredient[] =>
		ingredients.filter((ingredient) => ingredient.name.toLowerCase().includes(query.toLowerCase()))
};
