<script lang="ts">
		...
		return [...ingredients].sort((a, b) => {
			...
			switch (column) {
				...
				case 'partiesCount':
					...
					break;
				case 'bestDishName':
					const bestPartyDishA = a.bestPartyDishId
						? Data.getPartyDishById(a.bestPartyDishId)
						: null;
					const bestPartyDishB = b.bestPartyDishId
						? Data.getPartyDishById(b.bestPartyDishId)
						: null;
	...
	$: visibleIngredients = enrichedIngredients.filter(
		(ing) =>
			ingredientMatchesQuery(ing, searchQuery) &&
			(!showTrackedOnly || $trackedIngredientIds.has(ing.id))
	);
